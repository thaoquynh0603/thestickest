...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      ...
      paymentAmount: (latestPayment.payment_amount ?? 0) / 100,
      ...
      discountAmount: (latestPayment.discount_amount ?? 0) / 100,
      netAmount: (latestPayment.net_amount ?? 0) / 100,
      processingFee: (latestPayment.processing_fee_amount ?? 0) / 100,
